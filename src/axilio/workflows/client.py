# This file was auto-generated by Fern from our API Definition.

import datetime as dt
import typing

from ..core.client_wrapper import AsyncClientWrapper, SyncClientWrapper
from ..core.request_options import RequestOptions
from ..types.message_output_body import MessageOutputBody
from ..types.workflow_get_code_response import WorkflowGetCodeResponse
from ..types.workflow_list_revisions_response import WorkflowListRevisionsResponse
from ..types.workflow_revision_detail import WorkflowRevisionDetail
from ..types.workflow_save_code_response import WorkflowSaveCodeResponse
from ..types.workflow_workflow_create_response import WorkflowWorkflowCreateResponse
from ..types.workflow_workflow_from_code_response import WorkflowWorkflowFromCodeResponse
from ..types.workflow_workflow_list_response import WorkflowWorkflowListResponse
from ..types.workflow_workflow_response import WorkflowWorkflowResponse
from ..types.workflow_workflow_sort_spec import WorkflowWorkflowSortSpec
from .raw_client import AsyncRawWorkflowsClient, RawWorkflowsClient

# this is used as the default value for optional parameters
OMIT = typing.cast(typing.Any, ...)


class WorkflowsClient:
    def __init__(self, *, client_wrapper: SyncClientWrapper):
        self._raw_client = RawWorkflowsClient(client_wrapper=client_wrapper)

    @property
    def with_raw_response(self) -> RawWorkflowsClient:
        """
        Retrieves a raw implementation of this client that returns raw responses.

        Returns
        -------
        RawWorkflowsClient
        """
        return self._raw_client

    def list(
        self,
        *,
        limit: typing.Optional[int] = None,
        offset: typing.Optional[int] = None,
        search: typing.Optional[str] = None,
        status: typing.Optional[typing.Sequence[str]] = None,
        platform: typing.Optional[typing.Sequence[str]] = None,
        created_after: typing.Optional[str] = None,
        created_before: typing.Optional[str] = None,
        last_run_after: typing.Optional[str] = None,
        last_run_before: typing.Optional[str] = None,
        request_options: typing.Optional[RequestOptions] = None,
    ) -> WorkflowWorkflowListResponse:
        """
        Paginated list of workflows in the caller's org. Filters via query params: search, status, platform. Use POST /workflows/list for richer body-shaped queries with sort.

        Parameters
        ----------
        limit : typing.Optional[int]

        offset : typing.Optional[int]

        search : typing.Optional[str]
            free-text search across workflow name

        status : typing.Optional[typing.Sequence[str]]
            filter by workflow status (lowercase)

        platform : typing.Optional[typing.Sequence[str]]
            filter by device platform (lowercase)

        created_after : typing.Optional[str]
            only workflows created at/after this RFC3339 time

        created_before : typing.Optional[str]
            only workflows created at/before this RFC3339 time

        last_run_after : typing.Optional[str]
            only workflows last run at/after this RFC3339 time

        last_run_before : typing.Optional[str]
            only workflows last run at/before this RFC3339 time

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowWorkflowListResponse
            OK

        Examples
        --------
        from axilio import AxilioApi

        client = AxilioApi(
            api_key="YOUR_API_KEY",
        )
        client.workflows.list()
        """
        _response = self._raw_client.list(
            limit=limit,
            offset=offset,
            search=search,
            status=status,
            platform=platform,
            created_after=created_after,
            created_before=created_before,
            last_run_after=last_run_after,
            last_run_before=last_run_before,
            request_options=request_options,
        )
        return _response.data

    def create(
        self,
        *,
        name: str,
        goal: typing.Optional[str] = OMIT,
        ocr_engine: typing.Optional[str] = OMIT,
        platform: typing.Optional[str] = OMIT,
        request_options: typing.Optional[RequestOptions] = None,
    ) -> WorkflowWorkflowCreateResponse:
        """
        Creates a new workflow in the caller's org. Name must match ^[A-Za-z0-9_-]+$ and be unique within the org. Returns the workflow_id.

        Parameters
        ----------
        name : str

        goal : typing.Optional[str]

        ocr_engine : typing.Optional[str]

        platform : typing.Optional[str]

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowWorkflowCreateResponse
            Created

        Examples
        --------
        from axilio import AxilioApi

        client = AxilioApi(
            api_key="YOUR_API_KEY",
        )
        client.workflows.create(
            name="name",
        )
        """
        _response = self._raw_client.create(
            name=name, goal=goal, ocr_engine=ocr_engine, platform=platform, request_options=request_options
        )
        return _response.data

    def create_from_code(
        self,
        *,
        code_source: str,
        name: str,
        goal: typing.Optional[str] = OMIT,
        platform: typing.Optional[str] = OMIT,
        request_options: typing.Optional[RequestOptions] = None,
    ) -> WorkflowWorkflowFromCodeResponse:
        """
        Creates a workflow and its first code revision in a single transaction — the code playground's 'save as workflow'. Name must match ^[A-Za-z0-9_-]+$ and be unique within the org; platform defaults to ANDROID. Returns the workflow_id and revision_id.

        Parameters
        ----------
        code_source : str

        name : str

        goal : typing.Optional[str]

        platform : typing.Optional[str]

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowWorkflowFromCodeResponse
            Created

        Examples
        --------
        from axilio import AxilioApi

        client = AxilioApi(
            api_key="YOUR_API_KEY",
        )
        client.workflows.create_from_code(
            code_source="code_source",
            name="name",
        )
        """
        _response = self._raw_client.create_from_code(
            code_source=code_source, name=name, goal=goal, platform=platform, request_options=request_options
        )
        return _response.data

    def list_query(
        self,
        *,
        limit: int,
        offset: int,
        created_after: typing.Optional[str] = OMIT,
        created_before: typing.Optional[str] = OMIT,
        last_run_after: typing.Optional[str] = OMIT,
        last_run_before: typing.Optional[str] = OMIT,
        platform_filter: typing.Optional[typing.Sequence[str]] = OMIT,
        search: typing.Optional[str] = OMIT,
        sort_by: typing.Optional[typing.Sequence[WorkflowWorkflowSortSpec]] = OMIT,
        status_filter: typing.Optional[typing.Sequence[str]] = OMIT,
        request_options: typing.Optional[RequestOptions] = None,
    ) -> WorkflowWorkflowListResponse:
        """
        Paginated list of workflows in the caller's org with full filter + sort options in the request body.

        Parameters
        ----------
        limit : int

        offset : int

        created_after : typing.Optional[str]

        created_before : typing.Optional[str]

        last_run_after : typing.Optional[str]

        last_run_before : typing.Optional[str]

        platform_filter : typing.Optional[typing.Sequence[str]]

        search : typing.Optional[str]

        sort_by : typing.Optional[typing.Sequence[WorkflowWorkflowSortSpec]]

        status_filter : typing.Optional[typing.Sequence[str]]

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowWorkflowListResponse
            OK

        Examples
        --------
        from axilio import AxilioApi

        client = AxilioApi(
            api_key="YOUR_API_KEY",
        )
        client.workflows.list_query(
            limit=1000000,
            offset=1000000,
        )
        """
        _response = self._raw_client.list_query(
            limit=limit,
            offset=offset,
            created_after=created_after,
            created_before=created_before,
            last_run_after=last_run_after,
            last_run_before=last_run_before,
            platform_filter=platform_filter,
            search=search,
            sort_by=sort_by,
            status_filter=status_filter,
            request_options=request_options,
        )
        return _response.data

    def get(
        self, workflow_id: str, *, request_options: typing.Optional[RequestOptions] = None
    ) -> WorkflowWorkflowResponse:
        """
        Returns a single workflow, scoped to the caller's org (workflows in other orgs return 404).

        Parameters
        ----------
        workflow_id : str
            workflow identifier

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowWorkflowResponse
            OK

        Examples
        --------
        from axilio import AxilioApi

        client = AxilioApi(
            api_key="YOUR_API_KEY",
        )
        client.workflows.get(
            workflow_id="workflowID",
        )
        """
        _response = self._raw_client.get(workflow_id, request_options=request_options)
        return _response.data

    def update(
        self,
        workflow_id: str,
        *,
        name: typing.Optional[str] = OMIT,
        ocr_engine: typing.Optional[str] = OMIT,
        platform: typing.Optional[str] = OMIT,
        status: typing.Optional[str] = OMIT,
        request_options: typing.Optional[RequestOptions] = None,
    ) -> WorkflowWorkflowResponse:
        """
        Applies a partial update (name, platform, status, ocr_engine). Org-scoped — workflows in other orgs return 404.

        Parameters
        ----------
        workflow_id : str
            workflow identifier

        name : typing.Optional[str]

        ocr_engine : typing.Optional[str]

        platform : typing.Optional[str]

        status : typing.Optional[str]

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowWorkflowResponse
            OK

        Examples
        --------
        from axilio import AxilioApi

        client = AxilioApi(
            api_key="YOUR_API_KEY",
        )
        client.workflows.update(
            workflow_id="workflowID",
        )
        """
        _response = self._raw_client.update(
            workflow_id,
            name=name,
            ocr_engine=ocr_engine,
            platform=platform,
            status=status,
            request_options=request_options,
        )
        return _response.data

    def delete(self, workflow_id: str, *, request_options: typing.Optional[RequestOptions] = None) -> MessageOutputBody:
        """
        Deletes a workflow. Org-scoped — workflows in other orgs return 404.

        Parameters
        ----------
        workflow_id : str
            workflow identifier

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        MessageOutputBody
            OK

        Examples
        --------
        from axilio import AxilioApi

        client = AxilioApi(
            api_key="YOUR_API_KEY",
        )
        client.workflows.delete(
            workflow_id="workflowID",
        )
        """
        _response = self._raw_client.delete(workflow_id, request_options=request_options)
        return _response.data

    def get_code(
        self, workflow_id: str, *, request_options: typing.Optional[RequestOptions] = None
    ) -> WorkflowGetCodeResponse:
        """
        Returns the source code of the workflow's current revision, scoped to the caller's org.

        Parameters
        ----------
        workflow_id : str
            workflow identifier

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowGetCodeResponse
            OK

        Examples
        --------
        from axilio import AxilioApi

        client = AxilioApi(
            api_key="YOUR_API_KEY",
        )
        client.workflows.get_code(
            workflow_id="workflowID",
        )
        """
        _response = self._raw_client.get_code(workflow_id, request_options=request_options)
        return _response.data

    def save_code(
        self,
        workflow_id: str,
        *,
        source: str,
        message: typing.Optional[str] = OMIT,
        request_options: typing.Optional[RequestOptions] = None,
    ) -> WorkflowSaveCodeResponse:
        """
        Persists a new revision of the workflow's code. Hash-deduplicates against the current revision (no-op if unchanged). Source is capped at 256KB.

        Parameters
        ----------
        workflow_id : str
            workflow identifier

        source : str

        message : typing.Optional[str]

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowSaveCodeResponse
            OK

        Examples
        --------
        from axilio import AxilioApi

        client = AxilioApi(
            api_key="YOUR_API_KEY",
        )
        client.workflows.save_code(
            workflow_id="workflowID",
            source="source",
        )
        """
        _response = self._raw_client.save_code(
            workflow_id, source=source, message=message, request_options=request_options
        )
        return _response.data

    def restore_revision(
        self, workflow_id: str, *, revision_id: str, request_options: typing.Optional[RequestOptions] = None
    ) -> WorkflowSaveCodeResponse:
        """
        Creates a new revision with the source of the named revision. Does NOT dedup against the current revision so the action is auditable in the revision history.

        Parameters
        ----------
        workflow_id : str
            workflow identifier

        revision_id : str

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowSaveCodeResponse
            OK

        Examples
        --------
        from axilio import AxilioApi

        client = AxilioApi(
            api_key="YOUR_API_KEY",
        )
        client.workflows.restore_revision(
            workflow_id="workflowID",
            revision_id="revision_id",
        )
        """
        _response = self._raw_client.restore_revision(
            workflow_id, revision_id=revision_id, request_options=request_options
        )
        return _response.data

    def list_revisions(
        self,
        workflow_id: str,
        *,
        limit: typing.Optional[int] = None,
        before: typing.Optional[int] = None,
        request_options: typing.Optional[RequestOptions] = None,
    ) -> WorkflowListRevisionsResponse:
        """
        Returns revision metadata (id, number, author, message, bytes, sha256, created_at) for the workflow, in reverse-chronological order. Use the `before` cursor to paginate older revisions.

        Parameters
        ----------
        workflow_id : str
            workflow identifier

        limit : typing.Optional[int]

        before : typing.Optional[int]
            cursor: return revisions older than this revision number

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowListRevisionsResponse
            OK

        Examples
        --------
        from axilio import AxilioApi

        client = AxilioApi(
            api_key="YOUR_API_KEY",
        )
        client.workflows.list_revisions(
            workflow_id="workflowID",
        )
        """
        _response = self._raw_client.list_revisions(
            workflow_id, limit=limit, before=before, request_options=request_options
        )
        return _response.data

    def get_revision(
        self, workflow_id: str, revision_id: str, *, request_options: typing.Optional[RequestOptions] = None
    ) -> WorkflowRevisionDetail:
        """
        Returns a single revision including its full source. Defense-in-depth: the revision must belong to the named workflow or it's treated as missing.

        Parameters
        ----------
        workflow_id : str
            workflow identifier

        revision_id : str
            revision identifier

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowRevisionDetail
            OK

        Examples
        --------
        from axilio import AxilioApi

        client = AxilioApi(
            api_key="YOUR_API_KEY",
        )
        client.workflows.get_revision(
            workflow_id="workflowID",
            revision_id="revisionID",
        )
        """
        _response = self._raw_client.get_revision(workflow_id, revision_id, request_options=request_options)
        return _response.data

    def update_run(
        self,
        workflow_id: str,
        *,
        last_run_at: typing.Optional[dt.datetime] = OMIT,
        request_options: typing.Optional[RequestOptions] = None,
    ) -> WorkflowWorkflowResponse:
        """
        Bumps the workflow's last_run_at timestamp after a run starts.

        Parameters
        ----------
        workflow_id : str
            workflow identifier

        last_run_at : typing.Optional[dt.datetime]

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowWorkflowResponse
            OK

        Examples
        --------
        from axilio import AxilioApi

        client = AxilioApi(
            api_key="YOUR_API_KEY",
        )
        client.workflows.update_run(
            workflow_id="workflowID",
        )
        """
        _response = self._raw_client.update_run(workflow_id, last_run_at=last_run_at, request_options=request_options)
        return _response.data


class AsyncWorkflowsClient:
    def __init__(self, *, client_wrapper: AsyncClientWrapper):
        self._raw_client = AsyncRawWorkflowsClient(client_wrapper=client_wrapper)

    @property
    def with_raw_response(self) -> AsyncRawWorkflowsClient:
        """
        Retrieves a raw implementation of this client that returns raw responses.

        Returns
        -------
        AsyncRawWorkflowsClient
        """
        return self._raw_client

    async def list(
        self,
        *,
        limit: typing.Optional[int] = None,
        offset: typing.Optional[int] = None,
        search: typing.Optional[str] = None,
        status: typing.Optional[typing.Sequence[str]] = None,
        platform: typing.Optional[typing.Sequence[str]] = None,
        created_after: typing.Optional[str] = None,
        created_before: typing.Optional[str] = None,
        last_run_after: typing.Optional[str] = None,
        last_run_before: typing.Optional[str] = None,
        request_options: typing.Optional[RequestOptions] = None,
    ) -> WorkflowWorkflowListResponse:
        """
        Paginated list of workflows in the caller's org. Filters via query params: search, status, platform. Use POST /workflows/list for richer body-shaped queries with sort.

        Parameters
        ----------
        limit : typing.Optional[int]

        offset : typing.Optional[int]

        search : typing.Optional[str]
            free-text search across workflow name

        status : typing.Optional[typing.Sequence[str]]
            filter by workflow status (lowercase)

        platform : typing.Optional[typing.Sequence[str]]
            filter by device platform (lowercase)

        created_after : typing.Optional[str]
            only workflows created at/after this RFC3339 time

        created_before : typing.Optional[str]
            only workflows created at/before this RFC3339 time

        last_run_after : typing.Optional[str]
            only workflows last run at/after this RFC3339 time

        last_run_before : typing.Optional[str]
            only workflows last run at/before this RFC3339 time

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowWorkflowListResponse
            OK

        Examples
        --------
        import asyncio

        from axilio import AsyncAxilioApi

        client = AsyncAxilioApi(
            api_key="YOUR_API_KEY",
        )


        async def main() -> None:
            await client.workflows.list()


        asyncio.run(main())
        """
        _response = await self._raw_client.list(
            limit=limit,
            offset=offset,
            search=search,
            status=status,
            platform=platform,
            created_after=created_after,
            created_before=created_before,
            last_run_after=last_run_after,
            last_run_before=last_run_before,
            request_options=request_options,
        )
        return _response.data

    async def create(
        self,
        *,
        name: str,
        goal: typing.Optional[str] = OMIT,
        ocr_engine: typing.Optional[str] = OMIT,
        platform: typing.Optional[str] = OMIT,
        request_options: typing.Optional[RequestOptions] = None,
    ) -> WorkflowWorkflowCreateResponse:
        """
        Creates a new workflow in the caller's org. Name must match ^[A-Za-z0-9_-]+$ and be unique within the org. Returns the workflow_id.

        Parameters
        ----------
        name : str

        goal : typing.Optional[str]

        ocr_engine : typing.Optional[str]

        platform : typing.Optional[str]

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowWorkflowCreateResponse
            Created

        Examples
        --------
        import asyncio

        from axilio import AsyncAxilioApi

        client = AsyncAxilioApi(
            api_key="YOUR_API_KEY",
        )


        async def main() -> None:
            await client.workflows.create(
                name="name",
            )


        asyncio.run(main())
        """
        _response = await self._raw_client.create(
            name=name, goal=goal, ocr_engine=ocr_engine, platform=platform, request_options=request_options
        )
        return _response.data

    async def create_from_code(
        self,
        *,
        code_source: str,
        name: str,
        goal: typing.Optional[str] = OMIT,
        platform: typing.Optional[str] = OMIT,
        request_options: typing.Optional[RequestOptions] = None,
    ) -> WorkflowWorkflowFromCodeResponse:
        """
        Creates a workflow and its first code revision in a single transaction — the code playground's 'save as workflow'. Name must match ^[A-Za-z0-9_-]+$ and be unique within the org; platform defaults to ANDROID. Returns the workflow_id and revision_id.

        Parameters
        ----------
        code_source : str

        name : str

        goal : typing.Optional[str]

        platform : typing.Optional[str]

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowWorkflowFromCodeResponse
            Created

        Examples
        --------
        import asyncio

        from axilio import AsyncAxilioApi

        client = AsyncAxilioApi(
            api_key="YOUR_API_KEY",
        )


        async def main() -> None:
            await client.workflows.create_from_code(
                code_source="code_source",
                name="name",
            )


        asyncio.run(main())
        """
        _response = await self._raw_client.create_from_code(
            code_source=code_source, name=name, goal=goal, platform=platform, request_options=request_options
        )
        return _response.data

    async def list_query(
        self,
        *,
        limit: int,
        offset: int,
        created_after: typing.Optional[str] = OMIT,
        created_before: typing.Optional[str] = OMIT,
        last_run_after: typing.Optional[str] = OMIT,
        last_run_before: typing.Optional[str] = OMIT,
        platform_filter: typing.Optional[typing.Sequence[str]] = OMIT,
        search: typing.Optional[str] = OMIT,
        sort_by: typing.Optional[typing.Sequence[WorkflowWorkflowSortSpec]] = OMIT,
        status_filter: typing.Optional[typing.Sequence[str]] = OMIT,
        request_options: typing.Optional[RequestOptions] = None,
    ) -> WorkflowWorkflowListResponse:
        """
        Paginated list of workflows in the caller's org with full filter + sort options in the request body.

        Parameters
        ----------
        limit : int

        offset : int

        created_after : typing.Optional[str]

        created_before : typing.Optional[str]

        last_run_after : typing.Optional[str]

        last_run_before : typing.Optional[str]

        platform_filter : typing.Optional[typing.Sequence[str]]

        search : typing.Optional[str]

        sort_by : typing.Optional[typing.Sequence[WorkflowWorkflowSortSpec]]

        status_filter : typing.Optional[typing.Sequence[str]]

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowWorkflowListResponse
            OK

        Examples
        --------
        import asyncio

        from axilio import AsyncAxilioApi

        client = AsyncAxilioApi(
            api_key="YOUR_API_KEY",
        )


        async def main() -> None:
            await client.workflows.list_query(
                limit=1000000,
                offset=1000000,
            )


        asyncio.run(main())
        """
        _response = await self._raw_client.list_query(
            limit=limit,
            offset=offset,
            created_after=created_after,
            created_before=created_before,
            last_run_after=last_run_after,
            last_run_before=last_run_before,
            platform_filter=platform_filter,
            search=search,
            sort_by=sort_by,
            status_filter=status_filter,
            request_options=request_options,
        )
        return _response.data

    async def get(
        self, workflow_id: str, *, request_options: typing.Optional[RequestOptions] = None
    ) -> WorkflowWorkflowResponse:
        """
        Returns a single workflow, scoped to the caller's org (workflows in other orgs return 404).

        Parameters
        ----------
        workflow_id : str
            workflow identifier

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowWorkflowResponse
            OK

        Examples
        --------
        import asyncio

        from axilio import AsyncAxilioApi

        client = AsyncAxilioApi(
            api_key="YOUR_API_KEY",
        )


        async def main() -> None:
            await client.workflows.get(
                workflow_id="workflowID",
            )


        asyncio.run(main())
        """
        _response = await self._raw_client.get(workflow_id, request_options=request_options)
        return _response.data

    async def update(
        self,
        workflow_id: str,
        *,
        name: typing.Optional[str] = OMIT,
        ocr_engine: typing.Optional[str] = OMIT,
        platform: typing.Optional[str] = OMIT,
        status: typing.Optional[str] = OMIT,
        request_options: typing.Optional[RequestOptions] = None,
    ) -> WorkflowWorkflowResponse:
        """
        Applies a partial update (name, platform, status, ocr_engine). Org-scoped — workflows in other orgs return 404.

        Parameters
        ----------
        workflow_id : str
            workflow identifier

        name : typing.Optional[str]

        ocr_engine : typing.Optional[str]

        platform : typing.Optional[str]

        status : typing.Optional[str]

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowWorkflowResponse
            OK

        Examples
        --------
        import asyncio

        from axilio import AsyncAxilioApi

        client = AsyncAxilioApi(
            api_key="YOUR_API_KEY",
        )


        async def main() -> None:
            await client.workflows.update(
                workflow_id="workflowID",
            )


        asyncio.run(main())
        """
        _response = await self._raw_client.update(
            workflow_id,
            name=name,
            ocr_engine=ocr_engine,
            platform=platform,
            status=status,
            request_options=request_options,
        )
        return _response.data

    async def delete(
        self, workflow_id: str, *, request_options: typing.Optional[RequestOptions] = None
    ) -> MessageOutputBody:
        """
        Deletes a workflow. Org-scoped — workflows in other orgs return 404.

        Parameters
        ----------
        workflow_id : str
            workflow identifier

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        MessageOutputBody
            OK

        Examples
        --------
        import asyncio

        from axilio import AsyncAxilioApi

        client = AsyncAxilioApi(
            api_key="YOUR_API_KEY",
        )


        async def main() -> None:
            await client.workflows.delete(
                workflow_id="workflowID",
            )


        asyncio.run(main())
        """
        _response = await self._raw_client.delete(workflow_id, request_options=request_options)
        return _response.data

    async def get_code(
        self, workflow_id: str, *, request_options: typing.Optional[RequestOptions] = None
    ) -> WorkflowGetCodeResponse:
        """
        Returns the source code of the workflow's current revision, scoped to the caller's org.

        Parameters
        ----------
        workflow_id : str
            workflow identifier

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowGetCodeResponse
            OK

        Examples
        --------
        import asyncio

        from axilio import AsyncAxilioApi

        client = AsyncAxilioApi(
            api_key="YOUR_API_KEY",
        )


        async def main() -> None:
            await client.workflows.get_code(
                workflow_id="workflowID",
            )


        asyncio.run(main())
        """
        _response = await self._raw_client.get_code(workflow_id, request_options=request_options)
        return _response.data

    async def save_code(
        self,
        workflow_id: str,
        *,
        source: str,
        message: typing.Optional[str] = OMIT,
        request_options: typing.Optional[RequestOptions] = None,
    ) -> WorkflowSaveCodeResponse:
        """
        Persists a new revision of the workflow's code. Hash-deduplicates against the current revision (no-op if unchanged). Source is capped at 256KB.

        Parameters
        ----------
        workflow_id : str
            workflow identifier

        source : str

        message : typing.Optional[str]

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowSaveCodeResponse
            OK

        Examples
        --------
        import asyncio

        from axilio import AsyncAxilioApi

        client = AsyncAxilioApi(
            api_key="YOUR_API_KEY",
        )


        async def main() -> None:
            await client.workflows.save_code(
                workflow_id="workflowID",
                source="source",
            )


        asyncio.run(main())
        """
        _response = await self._raw_client.save_code(
            workflow_id, source=source, message=message, request_options=request_options
        )
        return _response.data

    async def restore_revision(
        self, workflow_id: str, *, revision_id: str, request_options: typing.Optional[RequestOptions] = None
    ) -> WorkflowSaveCodeResponse:
        """
        Creates a new revision with the source of the named revision. Does NOT dedup against the current revision so the action is auditable in the revision history.

        Parameters
        ----------
        workflow_id : str
            workflow identifier

        revision_id : str

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowSaveCodeResponse
            OK

        Examples
        --------
        import asyncio

        from axilio import AsyncAxilioApi

        client = AsyncAxilioApi(
            api_key="YOUR_API_KEY",
        )


        async def main() -> None:
            await client.workflows.restore_revision(
                workflow_id="workflowID",
                revision_id="revision_id",
            )


        asyncio.run(main())
        """
        _response = await self._raw_client.restore_revision(
            workflow_id, revision_id=revision_id, request_options=request_options
        )
        return _response.data

    async def list_revisions(
        self,
        workflow_id: str,
        *,
        limit: typing.Optional[int] = None,
        before: typing.Optional[int] = None,
        request_options: typing.Optional[RequestOptions] = None,
    ) -> WorkflowListRevisionsResponse:
        """
        Returns revision metadata (id, number, author, message, bytes, sha256, created_at) for the workflow, in reverse-chronological order. Use the `before` cursor to paginate older revisions.

        Parameters
        ----------
        workflow_id : str
            workflow identifier

        limit : typing.Optional[int]

        before : typing.Optional[int]
            cursor: return revisions older than this revision number

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowListRevisionsResponse
            OK

        Examples
        --------
        import asyncio

        from axilio import AsyncAxilioApi

        client = AsyncAxilioApi(
            api_key="YOUR_API_KEY",
        )


        async def main() -> None:
            await client.workflows.list_revisions(
                workflow_id="workflowID",
            )


        asyncio.run(main())
        """
        _response = await self._raw_client.list_revisions(
            workflow_id, limit=limit, before=before, request_options=request_options
        )
        return _response.data

    async def get_revision(
        self, workflow_id: str, revision_id: str, *, request_options: typing.Optional[RequestOptions] = None
    ) -> WorkflowRevisionDetail:
        """
        Returns a single revision including its full source. Defense-in-depth: the revision must belong to the named workflow or it's treated as missing.

        Parameters
        ----------
        workflow_id : str
            workflow identifier

        revision_id : str
            revision identifier

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowRevisionDetail
            OK

        Examples
        --------
        import asyncio

        from axilio import AsyncAxilioApi

        client = AsyncAxilioApi(
            api_key="YOUR_API_KEY",
        )


        async def main() -> None:
            await client.workflows.get_revision(
                workflow_id="workflowID",
                revision_id="revisionID",
            )


        asyncio.run(main())
        """
        _response = await self._raw_client.get_revision(workflow_id, revision_id, request_options=request_options)
        return _response.data

    async def update_run(
        self,
        workflow_id: str,
        *,
        last_run_at: typing.Optional[dt.datetime] = OMIT,
        request_options: typing.Optional[RequestOptions] = None,
    ) -> WorkflowWorkflowResponse:
        """
        Bumps the workflow's last_run_at timestamp after a run starts.

        Parameters
        ----------
        workflow_id : str
            workflow identifier

        last_run_at : typing.Optional[dt.datetime]

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        WorkflowWorkflowResponse
            OK

        Examples
        --------
        import asyncio

        from axilio import AsyncAxilioApi

        client = AsyncAxilioApi(
            api_key="YOUR_API_KEY",
        )


        async def main() -> None:
            await client.workflows.update_run(
                workflow_id="workflowID",
            )


        asyncio.run(main())
        """
        _response = await self._raw_client.update_run(
            workflow_id, last_run_at=last_run_at, request_options=request_options
        )
        return _response.data
